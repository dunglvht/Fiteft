...
def combine_matrices(*matrices):
  mat = np.zeros((sum([i.shape[0] for i in matrices]),sum([i.shape[1] for i in matrices])))
  count = np.array([0,0])
  for i in matrices:
    mat[count[0]:i.shape[0]+count[0],count[1]:count[1]+i.shape[1]]= i
    count += [i.shape[0],i.shape[1]]
  return mat

def make_theo_correlation(alist):
  # alist = [1, 2, [3, 4], 305, 404]
  alist = [i if isinstance(i, list) else [i] for i in alist]
  result = [[], [], [], [], []]
  order = ['']
  ggF_u = pd.read_csv(f'{path}/global_data/correlation_theory/ggF/uncertainty.csv', index_col=[0, 1])
  VBF_u = pd.read_csv(f'{path}/global_data/correlation_theory/VBF/uncertainty.csv', index_col=[0, 1])
  VH_u = pd.read_csv(f'{path}/global_data/correlation_theory/VH/uncertainty.csv', index_col=[0, 1])
  for i in alist:
      if i[0] in range(99):
          if order[-1]!='ggF':
            order.append('ggF')
          if i[0] in ggF_u.index:
            result[0].append(ggF_u.loc[i].sum(axis=0).to_frame().T)
          elif str(i[0]) in ggF_u.index:
            result[0].append(ggF_u.loc[map(str, i)].sum(axis=0).to_frame().T)
          elif "+".join(map(str, i)) in ggF_u.index:
            result[0].append(ggF_u.loc["+".join(map(str, i))
                                  ].sum(axis=0).to_frame().T)
          else:
            result[0].append(pd.DataFrame(
                  [[np.nan]*ggF_u.shape[1]], columns=ggF_u.columns))
      if i[0] in range(200, 299):
          if order[-1]!='VBF':
            order.append('VBF')
          if i[0] in VBF_u.index:
              result[1].append(VBF_u.loc[i].sum(axis=0).to_frame().T)
          elif str(i[0]) in VBF_u.index:
              result[1].append(VBF_u.loc[map(str, i)].sum(axis=0).to_frame().T)
          elif "+".join(map(str, i)) in VBF_u.index:
              result[1].append(VBF_u.loc["+".join(map(str, i))
                                  ].sum(axis=0).to_frame().T)
          else:
              result[1].append(pd.DataFrame(
                  [[np.nan]*VBF_u.shape[1]], columns=VBF_u.columns))
      if i[0] in range(300, 599):
          if order[-1]!='VH':
            order.append('VH')
          if i[0] in VH_u.index:
              result[2].append(VH_u.loc[i].sum(axis=0).to_frame().T)
          elif str(i[0]) in VH_u.index:
              result[2].append(VH_u.loc[map(str, i)].sum(axis=0).to_frame().T)
          elif "+".join(map(str, i)) in VH_u.index:
              result[2].append(VH_u.loc["+".join(map(str, i))].sum(axis=0).to_frame().T)
          else:
              result[2].append(pd.DataFrame(
                  [[np.nan]*VH_u.shape[1]], columns=VH_u.columns))
      if i[0] in range(600, 899):
          if order[-1]!='top':
            order.append('top')
            result[3].append(1)
      if i[0] not in range(0,899) :
          if order[-1]!='whatever':
            order.append('whatever')
          result[4].append(1)
  results = []
  # return result
  # print(order)
  for Ni,i in enumerate(order[1:]):
    if i == 'ggF':
        df = pd.concat(result[0], axis=0)
        df = df.fillna(df.sum(axis=0, numeric_only=True))
        C = np.dot(df.iloc[:, 2:-1], df.iloc[:, 2:-1].T)
        results.append(pd.DataFrame(
            C/np.sqrt(np.diag(C).reshape(-1, 1).dot(np.diag(C).reshape(1, -1)))))
    if i == 'VBF':
        df = pd.concat(result[1], axis=0)
        df = df.fillna(df.sum(axis=0, numeric_only=True))
        C = np.dot(df.iloc[:, :-1], df.iloc[:, :-1].T)
        results.append(pd.DataFrame(
            C/np.sqrt(np.diag(C).reshape(-1, 1).dot(np.diag(C).reshape(1, -1)))))
    if i == 'VH':
        df = pd.concat(result[2], axis=0)
        df = df.fillna(df.sum(axis=0, numeric_only=True))
        C = np.dot(df.iloc[:, -2:], df.iloc[:, -2:].T)
        results.append(pd.DataFrame(
            C/np.sqrt(np.diag(C).reshape(-1, 1).dot(np.diag(C).reshape(1, -1)))))
    if i == 'top':
        results.append(pd.DataFrame(np.ones((len(result[3],len(result[3]))))))
    if i == 'whatever':
        results.append(pd.DataFrame(np.identity(len(result[4]))))
  print(result[4])
  return combine_matrices(*results)

def make_theo_correlation_2(alist):
  # alist = [1, 2, [3, 4], 305, 404]
  alist = [i if isinstance(i, list) else [i] for i in alist]
  result = []
  order = ['']
  ...
  for i in alist:
      if i[0] in range(99):
          if order[-1]!='ggF':
            order.append('ggF')
          if i[0] in ggF_u.index:
            result.append(ggF_u.loc[i].sum(axis=0).to_frame().T)
          elif str(i[0]) in ggF_u.index:
            result.append(ggF_u.loc[map(str, i)].sum(axis=0).to_frame().T)
          elif "+".join(map(str, i)) in ggF_u.index:
            result.append(ggF_u.loc["+".join(map(str, i))
                                  ].sum(axis=0).to_frame().T)
          else:
      ...
      if i[0] in range(200, 299):
          if order[-1]!='VBF':
            order.append('VBF')
          if i[0] in VBF_u.index:
              result.append(VBF_u.loc[i].sum(axis=0).to_frame().T)
          elif str(i[0]) in VBF_u.index:
              result.append(VBF_u.loc[map(str, i)].sum(axis=0).to_frame().T)
          elif "+".join(map(str, i)) in VBF_u.index:
              result.append(VBF_u.loc["+".join(map(str, i))
                                  ].sum(axis=0).to_frame().T)
          else:
              result.append(pd.DataFrame(
                  [[np.nan]*VBF_u.shape[1]], columns=VBF_u.columns))
      if i[0] in range(300, 599):
          if order[-1]!='VH':
            order.append('VH')
          if i[0] in VH_u.index:
              result.append(VH_u.loc[i].sum(axis=0).to_frame().T)
          elif str(i[0]) in VH_u.index:
              result.append(VH_u.loc[map(str, i)].sum(axis=0).to_frame().T)
          elif "+".join(map(str, i)) in VH_u.index:
              result.append(VH_u.loc["+".join(map(str, i))].sum(axis=0).to_frame().T)
          else:
              result.append(pd.DataFrame(
                  [[np.nan]*VH_u.shape[1]], columns=VH_u.columns))
      if i[0] in range(600, 899):
